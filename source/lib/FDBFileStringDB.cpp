...
bool FDBFileStringDB::WriteMySQL(const char* filename)
{
	...
	fprintf(outf, "CREATE TABLE IF NOT EXISTS `strings` (`guid` VARCHAR(50) PRIMARY KEY);\n");

	fprintf(outf, "/*ALTER TABLE `strings` DROP COLUMN `%s`*/\n", lang.c_str());
	fprintf(outf, "/*ALTER TABLE `strings` ADD COLUMN `%s` TEXT;*/\n\n", lang.c_str());
    ...
    while (run<end)
    {
		...
		if (strlen(ent->value)>0) 
			fprintf(outf,"INSERT IGNORE INTO `strings` (`guid`) VALUES ('%s');\n", EscapeMySQL_String(ent->key).c_str());
    }
    ...
    while (run<end)
    {
       ...
       std::string value = std::string(ent->value);
		if (!value.empty()) 
			fprintf(outf,"UPDATE `strings` SET `%s`='%s' where `guid`='%s';\n",lang.c_str(), EscapeMySQL_String(value).c_str(), EscapeMySQL_String(ent->key).c_str());
    }
